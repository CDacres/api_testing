require 'spec_helper'
require_relative '../../../som_postcodes/postcodes_object.rb'
...
describe 'Validate postcodes.io fields' do
  ...
  context 'single postcode' do
    ...
    before(:all) do
      ...
      @postcodes = postcodes_service.single_postcode_call('WD247NX')
    end
  ...
  context 'multiple postcodes' do
    ...
    before(:all) do
      postcodes_service = PostcodesService.new
      @postcodes_array = postcodes_service.multiple_postcode_service(['WD247NX', 'SW61SH'])
    end

    it 'should have a postcode which is a string' do
      @postcodes_array['result'].each do |i|
        expect(i['result']['postcode']).to be_a(String)
      end
    end

    it 'should have a postcode containing a space' do
      @postcodes_array['result'].each do |i|
        expect(i['result']['postcode']).to include(' ')
      end
    end
